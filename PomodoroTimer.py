...
class PomodoroTimer:
    ...
    def start_timer_thread(self):
        if not self.running:
            # To only have one timer running
    ...
    def start_timer(self):
        ...
        if timer_id == 1:
            ...
            if not self.stopped or self.skipped:
                # if the time passes or if we skip the loop, we're going to count up the pomodoro
                self.pomodoros += 1
        ...
        # Case n2
            ...
            if not self.stopped or self.skipped:
                self.tabs.select(0) # because after a short break we always go back to pomodoro
                self.start_timer()
        ...
        # Case n3
            ...
            if not self.stopped or self.skipped:
                self.tabs.select(0) # because after a long break we always go back to pomodoro
                self.start_timer()
